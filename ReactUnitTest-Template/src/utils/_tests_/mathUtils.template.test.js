...
describe("mathUtils", () => {
  ...
  test("should compute fibonacci sequence and verify mathematical properties", () => {
    ...
    // Test 1: Generate Fibonacci sequence 0–10
    ...
    // Test 2: Verify F(n) = F(n-1) + F(n-2)
    for (let i = 2; i <= 10; i++) {
      expect(fibonacci(i)).toBe(fibonacci(i-1) + fibonacci(i-2));
    }

    // Test 3: Negative numbers should throw error
  ...
  test("should find GCD and LCM with verification of mathematical relationship", () => {
    ...
    const a = 48;
    const b = 18;
    expect(gcd(a, b) * lcm(a, b)).toBe(Math.abs(a * b));
