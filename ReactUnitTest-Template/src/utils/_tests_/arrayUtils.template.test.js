...
describe("arrayUtils", () => {
  ...
  test("should return unique values from array", () => {
    ...
    // Test 2: Remove duplicate strings
      ...
      expect(uniqueNumbers).toEqual([1, 2, 3]);

      const strings = ["a", "b", "a"];
      const uniqueStrings = unique(strings);
      expect(uniqueStrings).toEqual(["a", "b"]);
    });
  ...
  test("should deeply flatten nested arrays and compute sum with range generation", () => {
    ...
    // Test 2: Verify flattened array
    expect(flattened).toEqual([1, 2, 3, 4, 5, 6]);

    // Test 3: Sum of numbers
    expect(sum(flattened)).toBe(21);

    // Test 4: Range comparison
    const generatedRange = range(1, 7);
    expect(flattened).toEqual(generatedRange);
  });
  ...
  test("should group objects by property and verify group integrity", () => {
    ...
    // Test 1: Group employees
    const grouped = groupBy(employees, "department");

    // Test 2: Verify number of groups
    expect(Object.keys(grouped)).toHaveLength(3);

    // Test 3: Engineering should have 3 employees
    expect(grouped["Engineering"]).toHaveLength(3);

    // Test 4: Total Engineering salary
    const engineeringSalaries = grouped["Engineering"].map(emp => emp.salary);
